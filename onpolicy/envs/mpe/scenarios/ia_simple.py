...
from onpolicy.envs.mpe.ia_core import World, Harvester, Transporter, Field
...
class Scenario(BaseScenario):
    ...
    def reward(self, trans):
        ...
        rew = - 0.1 * np.linalg.norm(trans.vel)
        return rew

    def observation(self, trans, world):
        self_state = np.concatenate([[trans.i], trans.pos, trans.vel, [trans.load], [float(trans.transporting)], [float(trans.unloading)], [float(trans.full)], [float(trans.empty)]])
        # print(self_state)
        harv_states = []
        for harv in world.harvesters:
            harv_state = np.concatenate([[harv.i], harv.pos, [harv.load], [float(harv.full)], [float(harv.moving)], [float(harv.transporting)], [float(harv.complete_task)]])
